...
class CetakLaporanSuratMasuk extends Controller
{
    public function index($bulan_awal, $bulan_akhir, $tahun)
    {
        ...
        $laporan = SuratMasuk::whereBetween('tanggal_surat', [$tanggal_awal, $tanggal_akhir])->latest()->get()->toArray();
        // share data to view
        $data = [
            'laporan' => $laporan,
            'nama_awal' => $nama_awal,
        ...
        $pdf = Pdf::loadView('cetak.surat-masuk', $data)->setPaper('A4', 'potrait');
        // download PDF file with download method
        return $pdf->download('Laporan Surat Masuk Periode ' . $nama_awal . ' - ' . $nama_akhir . ' ' . $tahun . '.pdf');
    }
}
